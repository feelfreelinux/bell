...
void Mpeg4Container::readTrex() {
  ...
  sampleDefsLen++;
  sampleDefs = (SampleDefaults*)realloc(sampleDefs,
                                        sampleDefsLen * sizeof(SampleDefaults));
  sampleDefTracks =
      (uint32_t*)realloc(sampleDefTracks, sampleDefsLen * sizeof(uint32_t));
...
void Mpeg4Container::readTrun(uint32_t atomSize, uint32_t moofOffset) {
  ...
  uint32_t offset =
      def->offset ? def->offset
                  : moofOffset;  // base offset is baseDataOffset or moofOffset
  ...
  i = chunksLen++;
  chunks =
      (Mpeg4ChunkRange*)realloc(chunks, chunksLen * sizeof(Mpeg4ChunkRange));
  chunks[i].count = 1;
  chunks[i].samples = sampleCnt;
  chunks[i].sampleDescriptionId = def->sampleDescriptionId;
  i = chunkOffsetsLen++;
  chunkOffsets = (Mpeg4ChunkOffset*)realloc(
      chunkOffsets, chunkOffsetsLen * sizeof(Mpeg4ChunkOffset));
  chunkOffsets[i] = offset;

  // add all samples' sizes from this trun
  i = sampleSizesLen;
  sampleSizesLen += sampleCnt;
  sampleSizes = (Mpeg4SampleSize*)realloc(
      sampleSizes, sampleSizesLen * sizeof(Mpeg4SampleSize));
  ...
  for (; i < sampleSizesLen; i++) {
    durations[j] =
        trunFlags.sampleDurationPresent ? readUint32() : def->duration;
  ...
  samples = (Mpeg4SampleRange*)realloc(samples,
                                       samplesLen * sizeof(Mpeg4SampleRange));
  prevDuration = 0;
