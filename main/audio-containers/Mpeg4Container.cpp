...
void Mpeg4Container::feed(const std::shared_ptr<bell::ByteStream>& stream,
                          uint32_t position) {
...
bool Mpeg4Container::parse() {
  ...
  while (!parsed && !error && !closed) {
    std::cout << "Trying to read atom header" << std::endl;
    readAtomHeader(size, (uint32_t&)type);
    std::cout << static_cast<uint32_t>(type) << std::endl;
    switch (type) {
      ...
      case AtomType::ATOM_MDAT:
        ...
        isSeekable = fragmentsLen || (chunksLen && chunkOffsetsLen &&
                                      samplesLen && sampleSizesLen);
...
bool Mpeg4Container::goToData() {
  ...
  if (pos == curFragment->start || pos >= curFragment->end) {
    ...
    while (pos >= curFragment->end &&
           curFragment < fragments + fragmentsLen - 1) {
